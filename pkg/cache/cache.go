...
func (cd *Cache) DeletePrefix(ctx context.Context, prefixKey string) (err error) {
	regexPrefix := prefixKey + "*"
	if cd.opt.LocalCache != nil {
		keys := cd.opt.LocalCache.GetAllKeys()
	...
	if cd.opt.Redis != nil {
		...
		return true
		`
		err = cd.opt.Redis.Do(ctx, "EVAL", luaScript, 0, regexPrefix).Err()
	}
	return
}
